...
class ModuleSettingArgument : ICommandArgument {
    ...
    override fun getMatch(toMatch: List<String>): MatchType {
        ...
        return if (module.settingGroups.flatMap { it.settings }.any{ it.getName().replace(" ", "").equals(toMatch[1], ignoreCase = true) }){
            MatchType.FULL
        } else if (module.settingGroups.flatMap { it.settings }.any{ it.getName().replace(" ", "").startsWith(toMatch[1], ignoreCase = true) }){
            MatchType.PARTIAL
        } else {
            MatchType.NONE
        }
    ...
    fun getSetting(module: Module, toMatch: String): Setting<*>? {
        return module.settingGroups.flatMap { it.settings }.firstOrNull { it.getName().replace(" ", "").lowercase() == toMatch.lowercase() }
    }
}
